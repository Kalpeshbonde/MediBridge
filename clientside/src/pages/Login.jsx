...
const Login = () => {
  ...
  // ── Role-based redirect ──────────────────────────────────────────
  // Wait for both token AND userData (loaded async by AppContext).
  // userData.role === "doctor"  →  /doctor-dashboard
  // userData.role === "patient" →  /
  useEffect(() => {
    if (token && userData) {
      navigate(userData.role === "doctor" ? "/doctor-dashboard" : "/");
    }
  }, [token, userData]);
        ...
        <div className="bg-white rounded-2xl shadow-xl shadow-gray-100 border border-gray-100 overflow-hidden">
          ...
          <form onSubmit={onSubmitHandler} className="px-8 py-7 space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full border border-gray-200 rounded-xl px-4 py-3 pr-11 text-sm text-gray-800
                             placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-primary/30
                             focus:border-primary transition-all"
                />
            ...
            <button type="submit" disabled={loading}
              className="w-full bg-primary text-white py-3.5 rounded-xl font-semibold text-sm tracking-wide
